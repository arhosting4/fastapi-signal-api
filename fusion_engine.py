...
from sentinel import check_news # اپ ڈیٹ شدہ امپورٹ
...
async def generate_final_signal(symbol: str, candles: list, timeframe: str):
    ...
    try:
        ...
        if core_signal == "wait" and len(closes) < 34:
            return { "status": "no-signal", "symbol": symbol, "signal": "wait", "reason": "Insufficient historical data.", "confidence": 50.0, "tier": "Tier 5 – Weak", "timeframe": timeframe, "price": closes[-1], "tp": None, "sl": None }
        ...
        if risk_status == "High":
            return { "status": "blocked", "symbol": symbol, "signal": "wait", "reason": f"Trading BLOCKED: {risk_reason}", "confidence": 0.0, "tier": "Tier 5 – Weak", "timeframe": timeframe, "price": closes[-1], "tp": None, "sl": None }

        # *** اہم تبدیلی: نیوز چیک کو فعال کریں ***
        ...
        if news_impact == "High":
            return { "status": "blocked", "symbol": symbol, "signal": "wait", "reason": f"Trading BLOCKED: {news_reason}", "confidence": 0.0, "tier": "Tier 5 – Weak", "timeframe": timeframe, "price": closes[-1], "tp": None, "sl": None }
