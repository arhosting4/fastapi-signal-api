...
def get_higher_timeframe_trend(candles: list) -> str:
    ...
    sma_fast, sma_slow = ta.sma(df['close'], length=20), ta.sma(df['close'], length=50)
...
async def generate_final_signal(symbol: str, candles: list, timeframe: str, should_save_active: bool = True):
    try:
        ...
        if news_data["impact"] == "High": return {"signal": "wait", "reason": news_data['reason']}
        core_signal_data = generate_core_signal(symbol, timeframe, candles)
        core_signal = core_signal_data["signal"]
        if core_signal == "wait": return {"signal": "wait", "reason": "Primary indicators suggest no clear opportunity."}
        current_price = candles[-1]['close']
        structure_analysis = analyze_market_structure(core_signal, current_price, candles)
        if structure_analysis["decision"] == "block": return {"signal": "wait", "reason": structure_analysis['reason']}
        zones = find_zones(candles)
        price_location = analyze_price_in_zones(current_price, zones)
        if core_signal == "buy" and price_location.get("in_supply"): return {"signal": "wait", "reason": f"BUY signal blocked by supply zone."}
        if core_signal == "sell" and price_location.get("in_demand"): return {"signal": "wait", "reason": f"SELL signal blocked by demand zone."}
        ...
        confidence = get_confidence(core_signal, pattern_data.get("type"), risk_assessment.get("status"), news_data["impact"], symbol, timeframe)
        confidence += structure_analysis["confidence_boost"]
        if (core_signal == "buy" and htf_trend == "up") or (core_signal == "sell" and htf_trend == "down"): confidence += 10
        sd_reason_part = ""
        if core_signal == "buy" and not price_location.get("in_supply"):
            for zone in zones.get("demand", []):
                if current_price > zone['top'] and (current_price - zone['top']) < (zone['top'] - zone['bottom']) * 2:
                    ...
                    break
        if core_signal == "sell" and not price_action.get("in_demand"):
        ...
        tp_multiplier, sl_multiplier = risk_assessment.get("tp_multiplier", 2.0), risk_assessment.get("sl_multiplier", 1.0)
        ...
        final_result = {"symbol": symbol, "timeframe": timeframe, "signal": core_signal, "reason": final_reason, "confidence": round(confidence, 2), "tier": tier, "price": current_price, "tp": tp, "sl": sl, "candles": candles}
