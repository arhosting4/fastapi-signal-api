...
import pandas as pd
import numpy as np # ATR کے لیے numpy درکار ہے
...
def find_optimal_tp_sl(df: pd.DataFrame, signal_type: str) -> Optional[Tuple[float, float]]:
    ...
    # اصلاح: کم سے کم فاصلے کے لیے ATR کا حساب لگائیں
    atr = _calculate_atr(df)
    min_distance = atr * 1.5  # کم سے کم فاصلہ ATR کا 1.5 گنا ہو گا
    ...
    for level in all_levels:
        ...
        if score >= MIN_CONFLUENCE_SCORE:
            if level > last_close and abs(level - last_close) > min_distance:
                if signal_type == 'buy': potential_tp_levels[level] = score
                else: potential_sl_levels[level] = score
            elif level < last_close and abs(level - last_close) > min_distance:
