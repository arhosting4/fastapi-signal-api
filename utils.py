...
CANDLE_COUNT = API_CONFIG["CANDLE_COUNT"] + 1  # ایک اضافی کینڈل تاکہ نامکمل کو ہٹا سکیں
...
async def fetch_twelve_data_ohlc(symbol: str) -> Optional[List[Candle]]:
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            completed_candles_raw = sorted_values[1:101]  # سب سے حالیہ کینڈل (جو نامکمل ہو سکتی ہے) ہٹا دیں
            enriched_candles = []
