...
def get_tradeable_pairs() -> List[str]:
    """
    ★★★ نیا فنکشن ★★★
    ہفتے کے دن کی بنیاد پر تمام قابلِ تجارت جوڑوں کی فہرست واپس کرتا ہے۔
    """
    today = datetime.utcnow().weekday()
    # ہفتہ (5) اور اتوار (6) کو صرف کرپٹو
    if today >= 5:
        return TRADING_PAIRS["CRYPTO_PAIRS"]
    # ہفتے کے باقی دنوں میں فاریکس اور کرپٹو دونوں
...
async def get_real_time_quotes(symbols: List[str]) -> Optional[Dict[str, Any]]:
    """
    ★★★ نیا کم خرچ والا فنکشن (اسکینر ٹیم کے لیے) ★★★
    ایک ہی API کال میں متعدد علامتوں کے لیے حقیقی وقت کے کوٹس حاصل کرتا ہے۔
    """
    if not symbols:
        return {}

    # اسکینر پول سے ایک کلید حاصل کریں
    ...
    try:
        ...
        # عارضی خرابی کی صورت میں کلید کو تھوڑی دیر کے لیے محدود کریں
        if api_key:
...
async def fetch_twelve_data_ohlc(symbol: str) -> Optional[List[Candle]]:
    """
    ★★★ اپ ڈیٹ شدہ (تجزیاتی ٹیم کے لیے) ★★★
    TwelveData API سے OHLC کینڈلز لاتا ہے۔ اب یہ تجزیاتی پول استعمال کرتا ہے۔
    """
    # تجزیاتی پول سے ایک کلید حاصل کریں
...
async def get_current_prices_from_api(symbols: List[str]) -> Optional[Dict[str, float]]:
    """
    ★★★ اپ ڈیٹ شدہ (نگرانی ٹیم کے لیے) ★★★
    دی گئی علامتوں کی فہرست کے لیے تازہ ترین قیمتیں حاصل کرتا ہے۔ اب یہ نگرانی پول استعمال کرتا ہے۔
    """
    if not symbols:
        return {}

    # نگرانی پول سے ایک کلید حاصل کریں
