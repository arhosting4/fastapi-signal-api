...
async def get_real_time_quotes(symbols: List[str]) -> Optional[Dict[str, Any]]:
    """
    دی گئی علامتوں کے لیے تازہ ترین کوٹس حاصل کرتا ہے۔
    یہ فنکشن سختی سے 'گارڈین' کیز استعمال کرتا ہے۔
    """
    if not symbols:
        return {}
    ...
    if not api_key:
        logger.error("نگرانی کے لیے کوئی گارڈین API کلید دستیاب نہیں۔")
        return None
...
async def fetch_twelve_data_ohlc(symbol: str) -> Optional[List[Candle]]:
    """
    OHLC کینڈلز لاتا ہے۔
    یہ فنکشن سختی سے 'ہنٹر' کیز استعمال کرتا ہے۔
    """
    api_key = key_manager.get_hunter_key()
    if not api_key:
        logger.warning(f"[{symbol}] OHLC کے لیے کوئی ہنٹر API کلید دستیاب نہیں۔")
        return None
